...
from core.experiments import (
    BEST_AB_CONFIG as BASE_BEST_AB_CONFIG,
    QUADRANT_ASSIGNMENT_MODES,
    QUADRANT_EXPERIMENTS as BASE_QUADRANT_EXPERIMENTS,
    QUADRANT_TTA_MODES,
)

CFG = build_pipeline_config(
    "quadrant_detector_ablation",
    {
...
BEST_AB_CONFIG = dict(BASE_BEST_AB_CONFIG)
QUADRANT_EXPERIMENTS = list(BASE_QUADRANT_EXPERIMENTS)
...
if CFG["sanity_mode"]:
    ...
    QUADRANT_EXPERIMENTS = [
        dict(
            QUADRANT_EXPERIMENTS[0],
            imgsz=CFG["sanity_imgsz"],
            epochs=CFG["sanity_epochs"],
        )
    ]
    QUADRANT_TTA_MODES = [False]

ensure_output_dirs(CFG)

print_run_header(CFG, experiment_count=len(QUADRANT_EXPERIMENTS))
...
def build_quadrant_only_predictions(
    split_name: str,
    image_paths: List[str],
    image_id_lookup: Dict[str, int],
    quadrant_model: YOLO,
            ...
            predictions.append(
                {
                    ...
                    "quadrant": int(box.cls_id),
                }
            )

    print(f"{split_name} quadrant-only prediction count: {len(predictions)}")
    return predictions
...
def build_predictions_with_model_c(
    ...
    print(f"{split_name} A+B+C prediction count: {len(predictions)}")
    return predictions


def quadrant_only_metric_row(
    exp: dict, tta_enabled: bool, metrics: Dict[str, float], n_predictions: int
) -> dict:
    return {
        "experiment": exp["name"],
        "quadrant_model": exp["quadrant_model"],
        "imgsz": exp["imgsz"],
        "mosaic": exp["mosaic"],
        "scale": exp["scale"],
        "degrees": exp["degrees"],
        "quadrant_tta": tta_enabled,
        "n_predictions": n_predictions,
        "quadrant_AP": metrics["AP"],
        "quadrant_AP50": metrics["AP50"],
        "quadrant_AP75": metrics["AP75"],
        "quadrant_AR": metrics["AR"],
    }


def model_c_pipeline_row(
    exp: dict,
    tta_enabled: bool,
    assignment_mode: str,
    metrics_by_task: Dict[str, Dict[str, float]],
    ...
    return {
        ...
        "degrees": exp["degrees"],
        "quadrant_tta": tta_enabled,
        "quadrant_assignment_mode": assignment_mode,
        "n_predictions": n_predictions,
        "quadrant_AP50": metrics_by_task["Quadrant"]["AP50"],
        "enumeration_AP50": metrics_by_task["Enumeration"]["AP50"],
        "diagnosis_AP50": metrics_by_task["Diagnosis"]["AP50"],
...
print("=" * 80)
print("Training fixed Model A and Model B with the selected best configuration")
print("=" * 80)
pathology_result, pathology_weights = train_yolo_baseline(
    model_source=BEST_AB_CONFIG["pathology_model"],
    run_name=f"{BEST_AB_CONFIG['name']}_pathology",
    data_yaml=pathology_yaml,
    exp_cfg=BEST_AB_CONFIG,
)
tooth_result, tooth_weights = train_yolo_baseline(
    model_source=BEST_AB_CONFIG["tooth_model"],
    run_name=f"{BEST_AB_CONFIG['name']}_tooth",
...
quadrant_only_rows = []
pipeline_rows = []
quadrant_predictions_by_key = {}
pipeline_predictions_by_key = {}
quadrant_weights_by_experiment = {}

for exp in QUADRANT_EXPERIMENTS:
    print("=" * 80)
    print(f"Training Model C: {exp['name']}")
    print("=" * 80)
    quadrant_result, quadrant_weights = train_yolo_baseline(
        model_source=exp["quadrant_model"],
        run_name=f"{exp['name']}_quadrant",
        data_yaml=quadrant_yaml,
        exp_cfg=exp,
    )
    quadrant_weights_by_experiment[exp["name"]] = str(quadrant_weights)
    quadrant_model = YOLO(str(quadrant_weights))

    for quadrant_tta in QUADRANT_TTA_MODES:
        sanity_prefix = f"{exp['name']}_tta{int(quadrant_tta)}_sanity"
        quadrant_only_predictions = build_quadrant_only_predictions(
            split_name=sanity_prefix,
            ...
            enable_tta=quadrant_tta,
        )
        quadrant_metrics = evaluate_task_quiet(
            gt_payload=gt_eval_payload,
            predictions=quadrant_only_predictions,
            task_name="quadrant",
            category_field="category_id_1",
            categories_key="categories_1",
            prefix=sanity_prefix,
        )
        quadrant_only_rows.append(
            quadrant_only_metric_row(
                exp=exp,
                tta_enabled=quadrant_tta,
                metrics=quadrant_metrics,
                n_predictions=len(quadrant_only_predictions),
            )
        )
        quadrant_predictions_by_key[sanity_prefix] = quadrant_only_predictions

        for assignment_mode in QUADRANT_ASSIGNMENT_MODES:
            prefix = f"{exp['name']}_tta{int(quadrant_tta)}_{assignment_mode}"
            pipeline_predictions = build_predictions_with_model_c(
                split_name=prefix,
                image_paths=validation_image_paths,
                image_id_lookup=file_to_image_val,
                ...
                quadrant_conf=CFG["quadrant_conf"],
                predict_iou=CFG["predict_iou"],
                enable_ab_tta=BEST_AB_CONFIG["enable_tta"],
                enable_quadrant_tta=quadrant_tta,
                tooth_assignment_mode=BEST_AB_CONFIG["assignment_mode"],
                quadrant_assignment_mode=assignment_mode,
            )
            ...
            pipeline_rows.append(
                model_c_pipeline_row(
                    exp=exp,
                    tta_enabled=quadrant_tta,
                    assignment_mode=assignment_mode,
                    metrics_by_task=metrics_by_task,
                    n_predictions=len(pipeline_predictions),
                )
            )
            pipeline_predictions_by_key[prefix] = pipeline_predictions

quadrant_only_df = pd.DataFrame(quadrant_only_rows).sort_values(
    ["quadrant_AP50", "quadrant_AP", "quadrant_AR"], ascending=False
)
pipeline_df = pd.DataFrame(pipeline_rows).sort_values(
    ["aggregate_AP50", "quadrant_AP50", "aggregate_AP"], ascending=False
)

display(quadrant_only_df)
display(pipeline_df)

quadrant_only_df.to_csv(
    CFG["pred_root"] / "quadrant_only_sanity_check.csv", index=False
)
pipeline_df.to_csv(CFG["pred_root"] / "model_c_pipeline_comparison.csv", index=False)
write_json(
    CFG["pred_root"] / "quadrant_weights_by_experiment.json",
    quadrant_weights_by_experiment,
)

best_sanity_key = f"{quadrant_only_df.iloc[0]['experiment']}_tta{int(quadrant_only_df.iloc[0]['quadrant_tta'])}_sanity"
best_pipeline_key = (
    f"{pipeline_df.iloc[0]['experiment']}_"
    f"tta{int(pipeline_df.iloc[0]['quadrant_tta'])}_"
    f"{pipeline_df.iloc[0]['quadrant_assignment_mode']}"
)

best_quadrant_predictions = quadrant_predictions_by_key[best_sanity_key]
best_pipeline_predictions = pipeline_predictions_by_key[best_pipeline_key]

per_class_quadrant_df = evaluate_per_class_ap50(
    gt_payload=gt_eval_payload,
    predictions=best_quadrant_predictions,
    task_name="quadrant",
    category_field="category_id_1",
    categories_key="categories_1",
    prefix="best_model_c_quadrant_only",
)
per_class_quadrant_df.to_csv(
    CFG["pred_root"] / "best_model_c_per_class_quadrant_ap50.csv", index=False
)
display(per_class_quadrant_df)

best_error_df = error_decomposition(
    gt_eval_payload, best_pipeline_predictions, iou_thr=0.5
)
best_error_df.to_csv(
    CFG["pred_root"] / "best_model_c_error_decomposition.csv", index=False
)
display(
    best_error_df["error_type"]
    .value_counts()
...
write_json(
    CFG["pred_root"] / "best_model_c_predictions_validation.json",
    best_pipeline_predictions,
)

summary = {
    "best_quadrant_only_sanity": quadrant_only_df.iloc[0].to_dict(),
    "best_model_c_pipeline": pipeline_df.iloc[0].to_dict(),
    "best_quadrant_only_key": best_sanity_key,
    "best_pipeline_key": best_pipeline_key,
}
write_json(CFG["pred_root"] / "model_c_summary.json", summary)

print("Best quadrant-only sanity key:", best_sanity_key)
print("Best A+B+C pipeline key:", best_pipeline_key)
print(f"Saved outputs under: {CFG['pred_root']}")
prune_heavy_outputs(
    [
        pathology_weights,
        tooth_weights,
        *map(Path, quadrant_weights_by_experiment.values()),
    ]
)


summary = json.loads(
    (CFG["pred_root"] / "model_c_summary.json").read_text(encoding="utf-8")
)
