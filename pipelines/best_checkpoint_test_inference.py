...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run best-checkpoint inference on DENTEX test images."
    )
    parser.add_argument(
        "--max-test-images",
        type=int,
        default=None,
        help="Limit the number of test images processed.",
    )
    parser.add_argument(
        "--optimise-thresholds-ap75",
        action="store_true",
        help="Sweep validation thresholds and rank settings by aggregate AP75.",
    )
    parser.add_argument(
        "--pathology-thresholds",
        default="0.15,0.20,0.25,0.30,0.35,0.40,0.50,0.60",
        help="Comma-separated pathology confidence thresholds for the AP75 sweep.",
    )
    parser.add_argument(
        "--tooth-thresholds",
        default="0.15,0.25,0.35",
        help="Comma-separated tooth confidence thresholds for the AP75 sweep.",
    )
    parser.add_argument(
        "--quadrant-thresholds",
        default="0.15,0.25,0.35",
        help="Comma-separated quadrant confidence thresholds for the AP75 sweep.",
    )
    parser.add_argument(
        "--clean-review-output",
        action="store_true",
        help="Apply false-positive-control post-processing for visual review.",
    )
    parser.add_argument(
        "--compare-clean-review-validation",
        action="store_true",
        help="Compare standard and clean review settings on validation.",
    )
    parser.add_argument(
        "--clean-pathology-conf",
        type=float,
        default=0.30,
        help="Pathology confidence used by clean review output.",
    )
    parser.add_argument(
        "--clean-pathology-nms-iou",
        type=float,
        default=0.50,
        help="Class-agnostic pathology NMS IoU used by clean review output.",
    )
    parser.add_argument(
        "--clean-max-per-tooth",
        type=int,
        default=1,
        help="Maximum pathology predictions retained per assigned tooth.",
    )
...
from core.experiments import (
    BEST_MODEL_WEIGHTS,
    FINAL_CONFIG as BASE_FINAL_CONFIG,
)
...
def convert_split_to_yolo(
    ...
    for image in images:
        ...
        kept_image_paths.append(str(image_path.resolve()))
        if lines:
...
def write_prediction_files(
    ...
    pred_unified_path = (
        CFG["pred_root"] / f"{output_prefix}predictions_unified_{split_name}.json"
    )
    ...
    pred_challenge_path = (
        CFG["pred_root"]
        / f"{output_prefix}predictions_challenge_format_{split_name}.json"
    )
...
def class_agnostic_nms_boxes(
    boxes: List[DetectedBox],
    iou_thr: float,
) -> List[DetectedBox]:
    kept = []
    for box in sorted(boxes, key=lambda item: item.score, reverse=True):
        if all(iou_xyxy(box, kept_box) < iou_thr for kept_box in kept):
            kept.append(box)
    return kept


def limit_pathology_per_tooth(
    tooth_assignment: Dict[int, int],
    pathology_boxes: List[DetectedBox],
    max_per_tooth: int,
) -> Dict[int, int]:
    if max_per_tooth <= 0:
        return tooth_assignment

    grouped = defaultdict(list)
    for path_idx, tooth_idx in tooth_assignment.items():
        grouped[tooth_idx].append(path_idx)

    limited = {}
    for tooth_idx, path_indices in grouped.items():
        selected = sorted(
            path_indices,
            key=lambda path_idx: pathology_boxes[path_idx].score,
            reverse=True,
        )[:max_per_tooth]
        for path_idx in selected:
            limited[path_idx] = tooth_idx

    return limited


def build_challenge_boxes(predictions: List[dict]) -> List[dict]:
    challenge_boxes = []
    for prediction in predictions:
        ...
        challenge_boxes.append(
            {
                "name": (
                    f"{prediction['quadrant']} - "
                    f"{prediction['enumeration']} - "
                    f"{prediction['diagnosis']}"
                ),
                "corners": [
                    [float(x), float(y), image_id],
                    [float(x), float(y + height), image_id],
                    [float(x + width), float(y), image_id],
                    [float(x + width), float(y + height), image_id],
                ],
                "probability": float(prediction["score"]),
            }
        )
    return challenge_boxes


def build_clean_predictions_with_model_c(
    split_name: str,
    image_paths: List[str],
    image_id_lookup: Dict[str, int],
    pathology_model: YOLO,
    ...
    for image_path in image_paths:
        ...
        pathology_boxes = class_agnostic_nms_boxes(
            pathology_boxes, iou_thr=pathology_nms_iou
        )
        tooth_boxes = predict_boxes_with_tta(
            model=tooth_model,
            image_path=image_path,
        ...
        tooth_assignment = limit_pathology_per_tooth(
            tooth_assignment=tooth_assignment,
            pathology_boxes=pathology_boxes,
            max_per_tooth=max_per_tooth,
        )
        quadrant_assignment = assign_pathology_to_teeth_by_mode(
            pathology_boxes=pathology_boxes,
            tooth_boxes=quadrant_boxes,
            ...
            unified_predictions.append(
                {
                    ...
                    "bbox": xyxy_to_xywh(path_box),
                    "score": float(path_box.score),
                    "quadrant": int(quadrant_box.cls_id),
                    "enumeration": int(tooth_box.cls_id),
                    "diagnosis": int(diagnosis_id_map[path_box.cls_id]),
                    "quadrant_score": float(quadrant_box.score),
                    "tooth_score": float(tooth_box.score),
                }
    ...
    print(f"{split_name} clean A+B+C prediction count: {len(unified_predictions)}")
    return unified_predictions, build_challenge_boxes(unified_predictions)


def parse_thresholds(raw_thresholds: str) -> List[float]:
    return [float(value.strip()) for value in raw_thresholds.split(",")]


def filter_boxes_by_conf(
    boxes: List[DetectedBox], confidence: float
) -> List[DetectedBox]:
    return [box for box in boxes if box.score >= confidence]


def cache_model_c_boxes(
    image_paths: List[str],
    pathology_model: YOLO,
    tooth_model: YOLO,
    quadrant_model: YOLO,
    ...
    quadrant_conf: float,
    predict_iou: float,
) -> Dict[str, Dict[str, List[DetectedBox]]]:
    box_cache = {}

    for index, image_path in enumerate(image_paths, start=1):
        box_cache[image_path] = {
            "pathology": predict_boxes_with_tta(
                model=pathology_model,
                image_path=image_path,
                conf=pathology_conf,
                iou=predict_iou,
                enable_tta=FINAL_CONFIG["ab_enable_tta"],
            ),
            "tooth": predict_boxes_with_tta(
                model=tooth_model,
                image_path=image_path,
                conf=tooth_conf,
                iou=predict_iou,
            ...
            "quadrant": predict_boxes_with_tta(
                model=quadrant_model,
                image_path=image_path,
                conf=quadrant_conf,
                iou=predict_iou,
                enable_tta=FINAL_CONFIG["quadrant_enable_tta"],
            ),
        }
        print(f"Cached validation boxes: {index}/{len(image_paths)}")

    return box_cache


def build_predictions_with_model_c_cache(
    image_paths: List[str],
    image_id_lookup: Dict[str, int],
    box_cache: Dict[str, Dict[str, List[DetectedBox]]],
    diagnosis_id_map: Dict[int, int],
    pathology_conf: float,
    tooth_conf: float,
    quadrant_conf: float,
    ...
    for image_path in image_paths:
        ...
        image_id = int(image_id_lookup[file_name])
        cached_boxes = box_cache[image_path]
        pathology_boxes = filter_boxes_by_conf(
            cached_boxes["pathology"], pathology_conf
        )
        tooth_boxes = filter_boxes_by_conf(cached_boxes["tooth"], tooth_conf)
        quadrant_boxes = filter_boxes_by_conf(
            cached_boxes["quadrant"], quadrant_conf
        )
        ...
        tooth_assignment = assign_pathology_to_teeth_by_mode(
            pathology_boxes=pathology_boxes,
            tooth_boxes=tooth_boxes,
            assignment_mode=FINAL_CONFIG["tooth_assignment_mode"],
        )
        quadrant_assignment = assign_pathology_to_teeth_by_mode(
            pathology_boxes=pathology_boxes,
            tooth_boxes=quadrant_boxes,
            assignment_mode=FINAL_CONFIG["quadrant_assignment_mode"],
        )
...
def optimise_thresholds_for_ap75(args: argparse.Namespace) -> Path:
    ...
    file_to_image_validation = {
        image["file_name"]: int(image["id"])
        for image in gt_validation_payload["images"]
    }

    pathology_thresholds = parse_thresholds(args.pathology_thresholds)
    tooth_thresholds = parse_thresholds(args.tooth_thresholds)
    quadrant_thresholds = parse_thresholds(args.quadrant_thresholds)

    pathology_model, tooth_model, quadrant_model = load_best_models()
    box_cache = cache_model_c_boxes(
        image_paths=validation_image_paths,
        pathology_model=pathology_model,
        tooth_model=tooth_model,
        quadrant_model=quadrant_model,
        pathology_conf=min(pathology_thresholds),
        tooth_conf=min(tooth_thresholds),
        quadrant_conf=min(quadrant_thresholds),
    ...
    combinations = list(
        itertools.product(
            pathology_thresholds, tooth_thresholds, quadrant_thresholds
        )
    )
    for index, (pathology_conf, tooth_conf, quadrant_conf) in enumerate(
        combinations, start=1
    ):
        predictions = build_predictions_with_model_c_cache(
            image_paths=validation_image_paths,
            image_id_lookup=file_to_image_validation,
            box_cache=box_cache,
            diagnosis_id_map=diagnosis_id_map,
            pathology_conf=pathology_conf,
            tooth_conf=tooth_conf,
            quadrant_conf=quadrant_conf,
        )
        metrics_by_task = evaluate_all_tasks(
            gt_payload=gt_validation_payload,
            predictions=predictions,
            prefix=f"ap75_threshold_sweep_{index}",
        )
        aggregate = aggregate_task_metrics(metrics_by_task)
        rows.append(
            {
                ...
                "aggregate_AP75": aggregate["AP75"],
                "aggregate_AR": aggregate["AR"],
                "diagnosis_AP75": metrics_by_task["Diagnosis"]["AP75"],
                "enumeration_AP75": metrics_by_task["Enumeration"]["AP75"],
                "quadrant_AP75": metrics_by_task["Quadrant"]["AP75"],
            }
        )
        print(
            f"Evaluated threshold setting {index}/{len(combinations)}: "
            f"pathology={pathology_conf:.2f}, tooth={tooth_conf:.2f}, "
            f"quadrant={quadrant_conf:.2f}, aggregate AP75={aggregate['AP75']:.6f}"
        )

    threshold_df = pd.DataFrame(rows).sort_values(
        ["aggregate_AP75", "diagnosis_AP75", "aggregate_AP"],
        ascending=[False, False, False],
    )
    output_path = CFG["pred_root"] / "ap75_threshold_sweep.csv"
    threshold_df.to_csv(output_path, index=False)
    print(f"Saved AP75 threshold sweep: {output_path}")
    display(threshold_df.head(20))
    return output_path


def compare_clean_review_validation(args: argparse.Namespace) -> Path:
    gt_validation_payload = load_gt_payload("validation")
    diagnosis_id_map = diagnosis_pred_to_gt_id_map(gt_validation_payload)
    validation_image_paths = (
        (CFG["yolo_root"] / "pathology" / "validation.txt")
    ...
    pathology_model, tooth_model, quadrant_model = load_best_models()
    standard_predictions, _ = build_predictions_with_model_c(
        split_name="validation_standard",
        image_paths=validation_image_paths,
        image_id_lookup=file_to_image_validation,
        pathology_model=pathology_model,
        tooth_model=tooth_model,
    ...
    clean_predictions, _ = build_clean_predictions_with_model_c(
        split_name="validation_clean_review",
        image_paths=validation_image_paths,
        image_id_lookup=file_to_image_validation,
        pathology_model=pathology_model,
        tooth_model=tooth_model,
        quadrant_model=quadrant_model,
        diagnosis_id_map=diagnosis_id_map,
        pathology_conf=args.clean_pathology_conf,
        tooth_conf=CFG["tooth_conf"],
        quadrant_conf=CFG["quadrant_conf"],
        predict_iou=CFG["predict_iou"],
        pathology_nms_iou=args.clean_pathology_nms_iou,
        max_per_tooth=args.clean_max_per_tooth,
        enable_ab_tta=FINAL_CONFIG["ab_enable_tta"],
        enable_quadrant_tta=FINAL_CONFIG["quadrant_enable_tta"],
        tooth_assignment_mode=FINAL_CONFIG["tooth_assignment_mode"],
        quadrant_assignment_mode=FINAL_CONFIG["quadrant_assignment_mode"],
    ...
    for mode, predictions in [
        ...
        ("clean_review", clean_predictions),
    ]:
        metrics_by_task = evaluate_all_tasks(
            gt_payload=gt_validation_payload,
            predictions=predictions,
            prefix=f"{mode}_validation_comparison",
        )
        aggregate = aggregate_task_metrics(metrics_by_task)
        rows.append(
            {
                ...
                "pathology_conf": (
                    CFG["pathology_conf"]
                    if mode == "standard"
                    else args.clean_pathology_conf
                ),
                "tooth_conf": CFG["tooth_conf"],
                "quadrant_conf": CFG["quadrant_conf"],
                "pathology_nms_iou": (
                    np.nan if mode == "standard" else args.clean_pathology_nms_iou
                ),
                "max_per_tooth": (
                    np.nan if mode == "standard" else args.clean_max_per_tooth
                ),
                "predictions": len(predictions),
                "aggregate_AP": aggregate["AP"],
                "aggregate_AP50": aggregate["AP50"],
                ...
                "diagnosis_AP50": metrics_by_task["Diagnosis"]["AP50"],
                "diagnosis_AP75": metrics_by_task["Diagnosis"]["AP75"],
                "diagnosis_AR": metrics_by_task["Diagnosis"]["AR"],
            }
        )

    comparison_df = pd.DataFrame(rows)
    output_path = CFG["pred_root"] / "clean_review_validation_comparison.csv"
    comparison_df.to_csv(output_path, index=False)
    print(f"Saved clean review validation comparison: {output_path}")
...
BEST_MODEL_OUTPUT_ROOT = CFG["pred_root"]
BEST_MODEL_OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
ANNOTATED_IMAGE_ROOT = CFG["work_root"] / "annotated_images"


def load_best_models() -> Tuple[YOLO, YOLO, YOLO]:
    missing = [path for path in BEST_MODEL_WEIGHTS.values() if not Path(path).exists()]
    if missing:
        missing_text = "\n".join(str(path) for path in missing)
        raise FileNotFoundError(f"Missing best model weight file(s):\n{missing_text}")

    return (
        YOLO(str(BEST_MODEL_WEIGHTS["pathology"])),
        YOLO(str(BEST_MODEL_WEIGHTS["tooth"])),
        YOLO(str(BEST_MODEL_WEIGHTS["quadrant"])),
    )
...
def write_test_summary(
    ...
    annotated_image_dirs: Dict[str, Path],
) -> Path:
    payload = {
        "config": FINAL_CONFIG,
        "weights": {key: str(value) for key, value in BEST_MODEL_WEIGHTS.items()},
        "test_prediction_count": prediction_count,
        "prediction_files": {
            "test_unified": str(unified_path),
            "test_challenge": str(challenge_path),
        },
        "metrics_file": str(metrics_path) if metrics_path is not None else None,
        "annotated_image_dirs": {
            key: str(value) for key, value in annotated_image_dirs.items()
        },
    }
    summary_path = BEST_MODEL_OUTPUT_ROOT / "best_model_test_summary.json"
    write_json(summary_path, payload)
    return summary_path


def find_released_test_label_root() -> Path | None:
    candidates = [
        split_roots["test"] / "label",
        split_roots["test"] / "disease" / "label",
    ]
...
def save_annotated_model_predictions(
    model_name: str,
    model: YOLO,
    image_paths: List[str],
    conf: float,
    ...
    output_dir = ANNOTATED_IMAGE_ROOT / model_name
    ...
    print(f"Saved {saved} annotated {model_name} test images: {output_dir}")
    return output_dir


def write_clean_review_summary(
    prediction_count: int,
    unified_path: Path,
    challenge_path: Path,
    metrics_path: Path | None,
    ...
    summary_path = BEST_MODEL_OUTPUT_ROOT / "clean_review_test_summary.json"
    write_json(summary_path, payload)
    return summary_path
...
def main() -> None:
    args = parse_args()
    if args.optimise_thresholds_ap75:
        optimise_thresholds_for_ap75(args)
        return
    if args.compare_clean_review_validation:
        compare_clean_review_validation(args)
        return
    ...
    file_to_image_test = {
        Path(path).name: parse_test_image_id(Path(path).name)
    ...
    if args.max_test_images is not None:
        test_image_paths = test_image_paths[: args.max_test_images]

    print("Prediction-only best-model pipeline")
    print(f"Test images: {len(test_image_paths)}")
    print(f"Best weights: {BEST_MODEL_WEIGHTS}")
    print(f"Diagnosis id map: {diagnosis_id_map}")

    pathology_model, tooth_model, quadrant_model = load_best_models()
    annotated_image_dirs = {
        "pathology": save_annotated_model_predictions(
            model_name="pathology",
            model=pathology_model,
            image_paths=test_image_paths,
            conf=CFG["pathology_conf"],
            iou=CFG["predict_iou"],
            enable_tta=FINAL_CONFIG["ab_enable_tta"],
        ),
        "tooth": save_annotated_model_predictions(
            model_name="tooth",
            model=tooth_model,
            image_paths=test_image_paths,
            conf=CFG["tooth_conf"],
        ...
        "quadrant": save_annotated_model_predictions(
            model_name="quadrant",
            model=quadrant_model,
            image_paths=test_image_paths,
            conf=CFG["quadrant_conf"],
            iou=CFG["predict_iou"],
            enable_tta=FINAL_CONFIG["quadrant_enable_tta"],
        ),
    }

    if args.clean_review_output:
        test_predictions, test_challenge_boxes = build_clean_predictions_with_model_c(
            split_name="test",
            image_paths=test_image_paths,
    ...
    unified_path, challenge_path = write_prediction_files(
        split_name="test",
        unified_predictions=test_predictions,
        challenge_boxes=test_challenge_boxes,
        output_prefix=output_prefix,
    )
    print(f"Saved test unified predictions: {unified_path}")
    print(f"Saved test challenge-format predictions: {challenge_path}")

    metrics_path = None
    released_test_label_root = find_released_test_label_root()
    if released_test_label_root is not None:
        released_test_gt_payload = build_released_test_gt_payload(
            released_test_label_root
        )
        released_test_metrics = evaluate_all_tasks(
            gt_payload=released_test_gt_payload,
            predictions=test_predictions,
            prefix="best_model_released_test",
        )
        metrics_path = (
            BEST_MODEL_OUTPUT_ROOT
            / f"{output_prefix}best_model_test_metrics_official_style.json"
        )
        write_metrics_json(metrics_path, released_test_metrics)
        display(metric_table(released_test_metrics))
    else:
        print("Released-test labels not found")

    if args.clean_review_output:
        summary_path = write_clean_review_summary(
        ...
        summary_path = write_test_summary(
            prediction_count=len(test_predictions),
            unified_path=unified_path,
            challenge_path=challenge_path,
            metrics_path=metrics_path,
            annotated_image_dirs=annotated_image_dirs,
        )
    print(f"Saved summary: {summary_path}")
    print(f"Saved outputs under: {BEST_MODEL_OUTPUT_ROOT}")
