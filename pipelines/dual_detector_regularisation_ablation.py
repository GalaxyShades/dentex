...
from core.experiments import REGULARISATION_EXPERIMENTS

CFG = build_pipeline_config(
    "dual_detector_regularisation_ablation",
    {
        "weight_decay": 0.0005,
        "pathology_conf": 0.15,
        "tooth_conf": 0.25,
...
EXPERIMENTS = list(REGULARISATION_EXPERIMENTS)
...
def regularisation_result_row(
    ...
    return {
        ...
        "tta": exp["enable_tta"],
        "assignment_mode": exp["assignment_mode"],
        "pathology_conf": exp["pathology_conf"],
        "tooth_conf": exp["tooth_conf"],
        "predict_iou": exp["predict_iou"],
        "n_predictions": n_predictions,
        "quadrant_AP50": metrics_by_task["Quadrant"]["AP50"],
        "enumeration_AP50": metrics_by_task["Enumeration"]["AP50"],
        "diagnosis_AP50": metrics_by_task["Diagnosis"]["AP50"],
...
def save_prediction_files(
    prefix: str, predictions: List[dict], challenge_boxes: List[dict]
) -> None:
    write_json(
        CFG["pred_root"] / f"{prefix}_predictions_unified_validation.json", predictions
    )
    challenge_payload = {
        "name": "Regions of interest",
        "type": "Multiple 2D bounding boxes",
    ...
    write_json(
        CFG["pred_root"] / f"{prefix}_predictions_challenge_format_validation.json",
        challenge_payload,
    )
...
for exp in EXPERIMENTS:
    print("=" * 80)
    print(f"Running regularisation experiment: {exp['name']}")
    print("=" * 80)
    ...
    predictions, challenge_boxes = build_predictions_for_split(
    ...
    experiment_results.append(
        regularisation_result_row(
            exp=exp,
            metrics_by_task=metrics_by_task,
            n_predictions=len(predictions),
        )
    )
    experiment_predictions[exp["name"]] = predictions
    experiment_weights[exp["name"]] = {
        "pathology": str(pathology_weights),
        "tooth": str(tooth_weights),
    }
    save_prediction_files(exp["name"], predictions, challenge_boxes)

comparison_df = pd.DataFrame(experiment_results).sort_values(
    ["aggregate_AP50", "aggregate_AP", "aggregate_AR"],
    ascending=False,
)
print(comparison_df.to_string(index=False))
comparison_df.to_csv(CFG["pred_root"] / "regularisation_comparison.csv", index=False)
write_json(CFG["pred_root"] / "regularisation_weights.json", experiment_weights)

best_exp_name = str(comparison_df.iloc[0]["experiment"])
best_predictions = experiment_predictions[best_exp_name]
print(f"Best validation experiment by aggregate AP50: {best_exp_name}")

per_class_diagnosis_df = evaluate_per_class_ap50(
    gt_payload=gt_eval_payload,
    predictions=best_predictions,
    task_name="diagnosis",
    category_field="category_id_3",
    categories_key="categories_3",
    prefix=f"{best_exp_name}_diagnosis",
)
per_class_enumeration_df = evaluate_per_class_ap50(
    gt_payload=gt_eval_payload,
    predictions=best_predictions,
    task_name="enumeration",
    category_field="category_id_2",
    categories_key="categories_2",
    prefix=f"{best_exp_name}_enumeration",
)
per_class_quadrant_df = evaluate_per_class_ap50(
    ...
    category_field="category_id_1",
    categories_key="categories_1",
    prefix=f"{best_exp_name}_quadrant",
)

per_class_diagnosis_df.to_csv(
    CFG["pred_root"] / "best_per_class_diagnosis_ap50.csv", index=False
)
per_class_enumeration_df.to_csv(
    CFG["pred_root"] / "best_per_class_enumeration_ap50.csv", index=False
)
per_class_quadrant_df.to_csv(
    CFG["pred_root"] / "best_per_class_quadrant_ap50.csv", index=False
)

display(per_class_diagnosis_df)
display(per_class_enumeration_df)
display(per_class_quadrant_df)

error_df = error_decomposition(gt_eval_payload, best_predictions, iou_thr=0.5)
error_df.to_csv(CFG["pred_root"] / "best_error_decomposition.csv", index=False)
display(
    error_df["error_type"]
    .value_counts()
...
summary = {
    ...
    "comparison_file": str(CFG["pred_root"] / "regularisation_comparison.csv"),
}
write_json(CFG["pred_root"] / "regularisation_summary.json", summary)
print(json.dumps(summary, indent=2))
keep_weight_paths = [
    Path(path) for weights in experiment_weights.values() for path in weights.values()
]
prune_heavy_outputs(keep_weight_paths)


summary_path = CFG["pred_root"] / "regularisation_summary.json"
